...
from cpubone.apps.utils import dump_config, parse_unknown_args
from cpubone.clscore.data_provider import ImageNetDataProvider
from cpubone.clscore.trainer import ClsRunConfig, ClsTrainer
from cpubone.models.nn.drop import apply_drop_func
...
parser.add_argument("--resetoptimizer", action="store_true", default=False)
...
parser.add_argument("--auto_restart_thresh", type=float, default=2.5)
...
def main():
    ...
    print(config["data_provider"])
    
    if "total_batch_size" in config["data_provider"].keys():
        config["data_provider"]["base_batch_size"] = int(config["data_provider"]["total_batch_size"] / int(os.environ["WORLD_SIZE"]))
        
        if not "bsizemult" in config["run_config"].keys():
            config["run_config"]["bsizemult"] = 1
        else:
            config["data_provider"]["base_batch_size"] = int(config["data_provider"]["base_batch_size"] // config["run_config"]["bsizemult"])
        
        assert "total_lr" in config["run_config"].keys(), "total lr and total_batch_size need to be defined!"

        config["run_config"]["base_lr"] = config["run_config"]["total_lr"] / (int(os.environ["WORLD_SIZE"])  )
    ...
    model = get_cpubone(config_path=args.config, pretrained=False)
    
    try:
        apply_drop_func(model.backbone.stages, config["backbone_drop"])
    except: 
        print("Dropout couldn't be applied!")
    ...
    print("Training starts now!  Main LR:", config["run_config"]["init_lr"])
    # resume
    if args.resume:
        trainer.load_model(model_fname="model_best.pt" if args.evalnow else None, imagenetfinetune=args.imgnetfinetune, only_model=args.onlymodel, resetoptimizer=args.resetoptimizer)
        trainer.data_provider = setup.setup_data_provider(config, [ImageNetDataProvider], is_distributed=True)
    else:
        trainer.sync_model()
        # trainer.load_model(model_fname="/home/moritz/Research/paper_CPUBone/CPUBone/.exp/cls/imagenet/b1_grouping_deprecated/checkpoint/model_best.pt", imagenetfinetune=False, only_model=True)

    # launch training
    trainer.train(save_freq=args.save_freq, evalnow=args.evalnow)

import time
def mac_it(model, imgsize, timeit=False, chans=3, ret_macs=False):
    timstring=""
    if timeit and int(os.environ["LOCAL_RANK"]) == 0:
        starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
        bsize = 100
        device = "cuda:1"
        inp = torch.randn((bsize,chans,imgsize,imgsize)).to(device)
    ...
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model, (chans, imgsize, imgsize), as_strings=True,
