...
def zero_last_gamma(model: nn.Module, init_val=0) -> None:
    from cpubone_model import ResidualBlock, IdentityLayer, MBConv, FusedMBConv, ConvLayer

    for m in model.modules():
        if isinstance(m, ResidualBlock) and isinstance(m.shortcut, IdentityLayer):
            if isinstance(m.main, (MBConv, FusedMBConv)):
                parent_module = m.main.point_conv
            elif isinstance(m.main, ConvLayer):
