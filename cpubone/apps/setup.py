...
from cpubone.apps.data_provider import DataProvider
from cpubone.apps.trainer.run_config import RunConfig
from cpubone.apps.utils import (
    dist_init,
    dump_config,
    get_dist_local_rank,
    get_dist_rank,
...
from cpubone.models.utils import build_kwargs_from_config, load_state_dict_from_file
...
def setup_exp_config(config_path: str, recursive=True, opt_args: dict or None = None) -> dict:
    ...
    fpaths = ["configs/cls/imagenet/default.yml", config_path ]
...
def setup_data_provider(
    ...
    try:
        temp_total = int(os.environ["WORLD_SIZE"]) * dp_config["batch_size"] *  exp_config["run_config"]["bsizemult"]
        print("Batch size per instance:", dp_config["batch_size"], " | procs:", int(os.environ["WORLD_SIZE"]), " | bsizemult:", exp_config["run_config"]["bsizemult"], "total_bsize:",temp_total, "vs.", exp_config["data_provider"]["total_batch_size"] )
    except:
        print("No total_batch_size used!")
