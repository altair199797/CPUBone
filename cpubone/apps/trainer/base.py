...
from cpubone.apps.data_provider import DataProvider, parse_image_size
from cpubone.apps.trainer.run_config import RunConfig
from cpubone.apps.utils import EMA, dist_barrier, get_dist_local_rank, is_master
from cpubone.models.nn.norm import reset_bn
from cpubone.models.utils import is_parallel, load_state_dict_from_file
...
class Trainer:
    ...
    def load_model(self, model_fname=None, imagenetfinetune=False, only_model=False, resetoptimizer=False) -> None:
        latest_fname = os.path.join(self.checkpoint_path, "latest.txt")
        ...
        if imagenetfinetune:
            ...
            # print(self.network)
            missing, unexpected = self.network.load_state_dict(checkpoint["state_dict"], strict=True) # TODO CHANGE STRICT!
