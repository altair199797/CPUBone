...
from cpubone.apps.utils import AverageMeter
from cpubone.apps import setup

from cpubone.apps.utils import export_onnx
...
def other_macit(args, model):
    ...
    with get_accelerator().device(0):  
        flops, macs, params = get_model_profile(model=model, # model
                                    ...
                                    as_string=False, # print raw numbers (e.g. 1000) or as human-readable strings (e.g. 1k)
                                    output_file="b1_grouping.txt", # path to the output file. If None, the profiler prints to stdout.
                                    ignore_modules=None) # the list of modules to ignore in the profiling

    print(colored("Total Macs:","light_green"),"%f MMACS" % (macs/(1_000_000)))
...
def testrun_it_old(model, image_size, proc=-1, iterations=100, batch_size=300, cpu=False, return_dict=None, optit=False, args=None):
    ...
    print("backbonetime:", timings[-1],"| median: %.3f" % (np.median(timings)), "mean: %.3f" % (np.mean(timings)), "Throughput: %.3f" % (1000/np.median(timings)))
...
def get_model(modelname, other="", weight_url="", image_size=224, less_layers=0, args=None):
    ...
    if len(other) > 0:
        ...
        if "mambavision" in other:
            # from transformers import AutoModelForImageClassification
            # model = AutoModelForImageClassification.from_pretrained("nvidia/MambaVision-"+other.replace("mambavision","")+"-1K", trust_remote_code=True)
            from other_models.mambavision import get_model
            model = get_model(other.replace("mambavision",""))
        elif "fastvit" in other:
        ...
        elif "fasternet" in other:
            from other_models.fasternet import get_fasternet
            model = get_fasternet(other.replace("fasternet",""))
        ...
        model = get_cpubone(config_path="configs/cls/imagenet/"+modelname+".yaml", checkpoint_path=".exp/cls/imagenet/"+modelname+"/checkpoint/evalmodel.pt", pretrained=not args.testrun)
...
def convert_to_coreml(model, modelname, args):
    import coremltools as ct
    
    traceit = True 
    model.eval()
    
    example_inputs = (torch.rand(1, args.chaninp, args.image_size, args.image_size),)
    exported_program = torch.export.export(model, example_inputs, strict=False)
    ...
    if False:
        image_input = ct.ImageType(
            name="x",
            shape=example_inputs[0].shape,
            scale=1,
            bias=0,
        )
        
        print("input shape:", example_inputs[0].shape)
        ## COREML
        # CPU_ONLY, CPU_AND_GPU, CPU_AND_NE, ALL
        if traceit:
            model_conv = ct.convert(traced_model, inputs=[image_input], compute_units=ct.ComputeUnit.ALL)
        else:
            model_conv = ct.convert(exported_program, inputs=[image_input], compute_units=ct.ComputeUnit.ALL)
    else:
        image_input = ct.Shape(shape=(example_inputs[0].shape[0], example_inputs[0].shape[1], example_inputs[0].shape[2], example_inputs[0].shape[3]))
        image_input = ct.TensorType(shape=image_input, name="input")
        
        print("input shape:", example_inputs[0].shape)
        ## COREML
        # CPU_ONLY, CPU_AND_GPU, CPU_AND_NE, ALL
        if traceit:
            print("IT goes here and does","cpu" if args.coremlcpu else "all")
            model_conv = ct.convert(traced_model, inputs=[image_input], compute_units=ct.ComputeUnit.CPU_ONLY if args.coremlcpu else ct.ComputeUnit.ALL)
        else:
            model_conv = ct.convert(exported_program, inputs=[image_input], compute_units=ct.ComputeUnit.ALL)
    
    
    
    if not os.path.exists("Wcoreml_models"):
        os.makedirs("Wcoreml_models")
    # predict_with_coreml(model_conv, args)
    model_conv.save("Wcoreml_models/"+modelname+"_"+str(args.image_size)+".mlpackage")
    sys.exit()
...
def main():
    ...
    parser.add_argument("--path", type=str, default="/media/TBData4/data/imagenetsmall/imagenet-object-localization-challenge/ILSVRC/Data/CLS-LOC/val")
    ...
    parser.add_argument("--coremlcpu", action="store_true", default=False)
    parser.add_argument("--chaninp", type=int, default=3)
    parser.add_argument("--torch_conversion", action="store_true", default=False)
    ...
    ## run once
    model(torch.randn(1,3,args.image_size, args.image_size))
    
    if args.coreml:
        modelname = args.model if len(args.other)<1 else args.other
        convert_to_coreml(model, modelname, args)
        return
    ...
    with open("modelprint.txt", "w") as write_file:
        write_file.write(str(model))
    ...
    if args.torch_conversion:
        os.makedirs("Wtorchmodels", exist_ok=True)

        try:
            scripted_it = True
            with torch.no_grad():
                model = torch.jit.script(model.cuda(), example_inputs=[(1, args.chaninp, args.image_size, args.image_size)])#, example_inputs=[inp])    
        except:
            scripted_it = False
            print("Torchscript conversion failed! Uses jit_trace now!")
            model = torch.jit.trace(model.cuda(), example_inputs=torch.randn(1, args.chaninp, args.image_size, args.image_size).cuda())#, example_inputs=[inp])    
            
        # model = torch.jit.optimize_for_inference(model)
        modelname = args.model if len(args.other)<1 else args.other
        torchpath = os.path.join("Wtorchmodels",modelname+"_"+str(args.image_size)+(".tscript" if scripted_it else ".ttraced"))
        torch.jit.save(model,torchpath)
        print("jit conversion and saving successful!")
        return
    ...
    if args.onnxrun:
        ...
        os.makedirs("Wonnxmodels",exist_ok=True)
        temppath = os.path.join("Wonnxmodels",modelname+"_"+str(args.image_size)+".onnx")
        torch.onnx.export(model, inp, temppath, do_constant_folding=True,opset_version=args.opset, input_names=["input"], output_names=["output"], dynamic_axes={"input": {0: "batch_size"}, "output": {0: "batch_size"}})
        
        if False:
            import onnx
            from onnxsim import simplify as simplify_func
            
            onnx_model = onnx.load_model(temppath)
            onnx_model, success = simplify_func(onnx_model)
            assert success
            onnx.save(onnx_model,temppath)
            print("simplify worked!")
    ...
    ## reset BN TODO
    if False:
        from cpubone.models.nn.norm import reset_bn
...
# python eval_cls_model.py b1_testing7_other --gpu 4 --batch_size 1 --image_size 224  --testrun --iterations 4000  --latency --onnxrun 


#  python eval_cls_model.py b1_testing10 --gpu 0  --batch_size 20 --image_size 224 --testrun --other custom1 --cpu

#### ONNX Conversion
#  python eval_cls_model.py b1_grouping --gpu 6 --batch_size 1 --image_size 224  --testrun --iterations 4  --latency --onnxrun --cpu

"""

CUDA_VISIBLE_DEVICES=7 torchrun --nnodes 1 --nproc_per_node=1  --rdzv-endpoint localhost:29405 train_cls_model.py configs/cls/imagenet/b1_grouping_FullFusedv2_smkern.yaml      --data_provider.image_size "[128,160,192,224]"       --run_config.eval_image_size "[224]"     --path .exp/cls/imagenet/b1_grouping_FullFusedv2_smkern/ 


CUDA_VISIBLE_DEVICES=6,7 torchrun --nnodes 1 --nproc_per_node=2  --rdzv-endpoint localhost:29407 train_cls_model.py configs/cls/imagenet/b3_grouping_smallkernel.yaml      --data_provider.image_size "[128,160,192,224]"       --run_config.eval_image_size "[224]"     --path .exp/cls/imagenet/b3_grouping_smallkernel/  


CUDA_VISIBLE_DEVICES=2,3,4,5 torchrun --nnodes 1 --nproc_per_node=4  --rdzv-endpoint localhost:29403 train_cls_model.py configs/cls/imagenet/b15_grouping_smallkernel.yaml      --data_provider.image_size "[128,160,192,224]"       --run_config.eval_image_size "[224]"     --path .exp/cls/imagenet/b15_grouping_smallkernel/ 


CUDA_VISIBLE_DEVICES=3,2,4,5 torchrun --nnodes 1 --nproc_per_node=4  --rdzv-endpoint localhost:29411 train_cls_model.py configs/cls/imagenet/s0.yaml      --data_provider.image_size "[128,160,192,224]"       --run_config.eval_image_size "[224]"     --path .exp/cls/imagenet/s0/ 
